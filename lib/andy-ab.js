...
AndyAB.prototype.enrol = function enrol(callback) {
  if (this.enrolled()) { return this; }
  const cohort = this.assignCohort();
  if (typeof callback === 'function') { callback(cohort); }
  return this;
};

AndyAB.prototype.enrolled = function enrolled() {
  return !!this.getCohort();
};

AndyAB.prototype.assignCohort = function assignCohort() {
  const cohort = this.exclusionCohort() || this.sampleCohort();
  this.experimentCookie.setCohort(this.experimentName, cohort);
  return cohort;
};
...
AndyAB.prototype.getCohort = function getCohort() {
  return this.experimentCookie.getCohort(this.experimentName);
};
